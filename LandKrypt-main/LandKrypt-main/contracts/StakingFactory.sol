...
contract StakingFactory {
    ...
    function updateDevelopmentContract(address _developmentContract) external {
       require(msg.sender == owner, "Only owner can call this function");
       developmentContract = DevelopmentContract(_developmentContract);  
    }
    ...
    function createStakingContract(
        ...
        marketplace.listNFT(tokenId, listingPrice, stakingAddress, msg.sender);
        activeContracts[stakingAddress] = true;
    ...
    // getStakingContractForNFT for getting nft address
    ...
    function getStakingContractsPaginated(uint256 start, uint256 end) external view returns (address[] memory) {
        address[] memory active = new address[](stakingContracts.length);
        uint256 count = 0;
        
        for (uint256 i = 0; i < stakingContracts.length; i++) {
            if (activeContracts[stakingContracts[i]]) {
                active[count++] = stakingContracts[i];
            }
        }
        
        require(start <= end && end < count, "Invalid range");
   ...
   function recoverNFT(uint256 tokenId) external onlyOwner {
        ...
        activeContracts[stakingContract] = false;
