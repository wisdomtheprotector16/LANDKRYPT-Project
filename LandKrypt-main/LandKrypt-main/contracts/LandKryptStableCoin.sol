...
contract LandKryptStablecoin is ERC20, Ownable {
    ...
    constructor() 
       ERC20("LandKrypt Stablecoin", "LKUSD")
       Ownable(msg.sender) {}
    ...
    modifier onlyMinter() {
        require(_minters[msg.sender], "LandKryptStablecoin: caller is not a minter");
        _;
    }
     ...
     */
    modifier onlyBurner() {
        require(_burners[msg.sender], "LandKryptStablecoin: caller is not a burner");
        _;
    }
    ...
    function addMinter(address account) external onlyOwner {
        require(account != address(0), "LandKryptStablecoin: zero address cannot be minter");
        require(!_minters[account], "LandKryptStablecoin: address is already minter");
    ...
    function removeMinter(address account) external onlyOwner {
        require(_minters[account], "LandKryptStablecoin: address is not minter");
    ...
    function addBurner(address account) external onlyOwner {
        require(account != address(0), "LandKryptStablecoin: zero address cannot be burner");
        require(!_burners[account], "LandKryptStablecoin: address is already burner");
    ...
    function removeBurner(address account) external onlyOwner {
        require(_burners[account], "LandKryptStablecoin: address is not burner");
    ...
    /**
     * @dev Mint stablecoins (only callable by allowed minters)
    ...
    /**
     * @dev Burn stablecoins (only callable by allowed burners)
