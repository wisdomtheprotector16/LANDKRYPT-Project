...
contract NFTStaking is AutomationCompatibleInterface, ReentrancyGuard {
    ...
    uint256 public devContractCreationTime;
    ...
    function returnTokenId() public returns(uint256) {
        return tokenId;
    }
    ...
    function stake(uint256 amount) external nonReentrant {
        // Stacking address and nft contract
    ...
    function claimDailyRewards() public nonReentrant {
        ...
        require(devContractCreationTime > 0, "developement contract not created");
        ...
        if (daysPassed > 0) {
            ...
            if (reward > 0) {
                stablecoin.transfer(msg.sender, reward);//supposed to be .mint we 
    ...
    function recordDevelopmentContractMint() external{
        devContractCreationTime = block.timestamp / 1 days;
    }

    // Distribute final rewards when project is completed
        function distributeFinalRewards() public nonReentrant {
    ...
    function calculatePendingDailyRewards(address stakerAddress) public view returns (uint256) {
        ...
        if (devContractCreationTime > 0) {
            uint256 startDay = staker.lastClaimDay > devContractCreationTime ? 
                staker.lastClaimDay : devContractCreationTime;
            daysPassed = currentDay - startDay;
        } else {
            daysPassed = currentDay - staker.lastClaimDay;
        }
