...
const CustomConnectButton = () => {
  ...
  const handleAuthentication = async () => {
    ...
    try {
      ...
      const nonceResponse = await axios.post("http://localhost:8080/nonce", {
        address: address,
      });
      const { nonce } = nonceResponse.data;

      // 2. Format message as expected by backend
      const message = `Sign this nonce to authenticate: ${nonce}`;
      setAuthData((prev) => ({ ...prev, nonce, message }));

      // 3. Sign the message
      const signature = await signMessageAsync({ message });
      setAuthData((prev) => ({ ...prev, signature }));
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Enhanced Authentication Data Display */}
      <div className="bg-gray-100 p-4 rounded-lg space-y-4">
        <h3 className="font-bold text-lg">Authentication Debug Info</h3>
        
        {authData.loading && (
          <div className="text-blue-600 p-2 bg-blue-50 rounded">Authenticating...</div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {authData.nonce && (
            <div className="bg-white p-3 rounded border">
              <h4 className="font-semibold text-sm mb-1">Nonce</h4>
              <code className="text-xs break-all">{authData.nonce}</code>
            </div>
          )}
          ...
          {authData.message && (
            <div className="bg-white p-3 rounded border">
              <h4 className="font-semibold text-sm mb-1">Signed Message</h4>
              <code className="text-xs break-all">{authData.message}</code>
            </div>
          )}
          ...
          {authData.signature && (
            <div className="bg-white p-3 rounded border">
              ...
              <code className="text-xs break-all">{authData.signature}</code>
            </div>
          )}
          ...
          {authData.token && (
            <div className="bg-white p-3 rounded border col-span-full">
              <h4 className="font-semibold text-sm mb-1">JWT Token</h4>
              <code className="text-xs break-all">{authData.token}</code>
